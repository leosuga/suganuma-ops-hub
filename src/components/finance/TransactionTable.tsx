...
function TxnRowContent({ txn, onDelete, onEdit }: {
  txn: TransactionRow
  onDelete?: (id: string) => void
  onEdit?: (txn: TransactionRow) => void
}) {
  ...
  return (
    <div className="flex items-center gap-3 h-10 px-4 hover:bg-surface-hover transition-colors border-b border-border" role="row">
      <span className={cn("flex-none w-8 text-[8px] font-mono font-semibold tracking-wider", KIND_COLORS[txn.kind])} role="cell">
      ...
      <span className={cn("flex-none w-24 text-right text-[12px] font-mono font-semibold tabular-nums", KIND_COLORS[txn.kind])} role="cell">
...
export function TransactionTable({ transactions, isLoading, onDelete, onEdit }: TransactionTableProps) {
  const renderRow = useCallback((index: number) => {
    ...
    return <TxnRowContent txn={txn} onDelete={onDelete} onEdit={onEdit} />
  ...
  const useVirtual = transactions.length > 50
  ...
  return (
    <div className="border border-border bg-surface rounded-sm overflow-hidden" role="table" aria-label="Transações">
      ...
      {useVirtual ? (
        <VirtualizedList items={transactions} rowHeight={40} overscan={15} renderRow={renderRow} />
      ) : (
        <div className="divide-y divide-border" role="rowgroup">
          {transactions.map((txn) => (
            <TxnRowContent key={txn.id} txn={txn} onDelete={onDelete} onEdit={onEdit} />
          ))}
        </div>
      )}
    </div>
